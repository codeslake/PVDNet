...
%%%MM%%%
%%%MM%%% This version of the VSNR computation routine is identical to the
%%%MM%%% original routine, except that it calls a different function to
%%%MM%%% perform a wavelet decomposition.  The reason for this decision
%%%MM%%% is to improve the portability of the code.  The original VSNR
%%%MM%%% package includes Windows-compatible MEX hooks to code to compute
%%%MM%%% a 9/7 two-dimensional wavelet transform, while this one includes
%%%MM%%% a Matlab version that is used in situations where the existing
%%%MM%%% MEX hooks cannot be built properly.  Also, when the input image
%%%MM%%% image is converted to the luminance domain, it is first clipped
%%%MM%%% and quantized to an integer in the range [0,255]
%%%MM%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%MM%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%MM%%% [BEGIN CHANGE]
%%%MM%%%
%%%MM%%% force image to integers in the range [0,255]
%%%MM%%%

%%%MM%%%src_img_lum = vsnr_data.pix2lum_table(vsnr_data.src_img(:) + 256);
src_img_lum = vsnr_data.pix2lum_table( double(uint8( vsnr_data.src_img(:) )) + 256);
%%%MM%%%
%%%MM%%% [END CHANGE]
%%%MM%%%
...
%%%MM%%%
%%%MM%%% replace the original 'imdwt' function with another version
%%%MM%%%

% compute the image contrast at each scale
%%%MM%%%vsnr_data.src_bands = imdwt(vsnr_data.src_img, vsnr_data.num_levels);
...
if (vsnr_data.Ce > vsnr_data.CTe)    
  ...
  %%%MM%%%err_bands = imdwt(err_img, vsnr_data.num_levels);
  err_bands = imdwt_modified(err_img, vsnr_data.num_levels);
